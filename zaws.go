...
func NewZaws() *Zaws {
	...
	httpclient := &http.Client{
		Transport: &http.Transport{
			Proxy: func(*http.Request) (*url.URL, error) {
				return url.Parse("http://10.64.1.12:3128") // Or your own implementation that decides a proxy based on the URL in the request
			},
		},
	}
	...
	zaws.AwsSession = session.New(&aws.Config{
		...
		Credentials: credentials.NewStaticCredentials(zaws.AccessKeyId, zaws.SecretKeyId, zaws.Token),
...
func (z *Zaws) SetOption() {
	...
	flag.StringVar(&z.EC2Role, "ec2role", "", "Set role for EC2 Role")
	...
	if z.AccessKeyId == "" || z.SecretKeyId == "" {
		resp, err := http.Get("http://169.254.169.254/latest/meta-data/iam/security-credentials/" + z.EC2Role)
		if err != nil {
		...
		z.AccessKeyId = rdata.AccessKeyId
		z.SecretKeyId = rdata.SecretAccessKey
		z.Token = rdata.Token
	}
}
...
func usage() {
	fmt.Println("Usage: zaws service method [target] [-region|-r] [-key|-k] [-secret|-s] [-id|-i] [-metric|-m] [-host|h] [-port|p] [--ec2role]")
	os.Exit(1)
}
